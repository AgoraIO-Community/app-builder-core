    ...
    <View style={Platform.OS === 'web' ? style.chatView : style.chatViewNative}>
      ...
      <View style={style.chatNav}>
        <TouchableOpacity
          ...
          {pendingPublicNotification !== 0 ? <View style={style.chatNotification}>{pendingPublicNotification}</View> : null}
        ...
        <TouchableOpacity
          ...
          {pendingPrivateNotification !== 0 ? <View style={style.chatNotification}>{pendingPrivateNotification}</View> : null}
          <Text style={!groupActive ? style.groupTextActive : style.groupText}>
            Private
          </Text>
        </TouchableOpacity>
                      ...
                      if (user.uid !== 'local') {
                        return (
                          <TouchableOpacity
                            ...
                            onPress={() => {
                              selectUser(user)
                              setPrivateMessageLastSeen({ userId: user.uid, lastSeenCount: privateMessageCountMap[user.uid]})
                            }}>
                            {((privateMessageCountMap[user.uid] || 0) - (lastCheckedPrivateState[user.uid] || 0) !== 0) ? <View style={style.chatNotification}>{(privateMessageCountMap[user.uid] || 0) - (lastCheckedPrivateState[user.uid] || 0)}</View> : null}
