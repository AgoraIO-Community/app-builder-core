...
*********************************************
*/
import {LayoutComponent, useContent, useLayout} from 'customization-api';
import React, {useContext, useMemo, useState} from 'react';
import {View, StyleSheet, Pressable, Text} from 'react-native';
import {isWebInternal, useIsDesktop} from '../../src/utils/common';
...
const CustomGridLayout: LayoutComponent = ({renderData}) => {
  const localUid = useLocalUid();
  const {hideSelfView} = useCustomWrapper();
  const customRenderData = hideSelfView
    ? renderData?.filter(i => i !== localUid)
    : renderData;
  ...
  let {matrix, dims} = useMemo(
    () => layout(customRenderData.length, isDesktop()),
    [customRenderData.length, isDesktop()],
  );
    ...
    <View style={[style.full]}>
      {matrix.map((r, ridx) => (
        <View
          ...
          {r.map((c, cidx) => (
            <Pressable
              disabled={customRenderData.length === 1}
              onPress={() => {
                if (!(ridx === 0 && cidx === 0)) {
                  dispatch({
                    type: 'SwapVideo',
                    value: [customRenderData[ridx * dims.c + cidx]],
                  });
                }
                setLayout('pinned');
              }}
              ...
              <View
                ...
                <RenderComponent uid={customRenderData[ridx * dims.c + cidx]} />
              </View>
            </Pressable>
          ))}
        </View>
