...
const SurfaceView = (props: SurfaceViewInterface) => {
    ...
    useEffect(function () {
        ...
        {
            engine.setupViewContentMode(uid,1,props.channelId)
        }
        else
        {
            engine.setupViewContentMode(uid,0,props.channelId)
        }
        ...
        return ()=>{
            ...
            engine.setupRemoteVideo(props.uid==='local'?0:props.uid, false, props.channelId)
        }
    },[props.uid]);

    return <div id={String(props.uid)+'-player'} ref={canvasElem}  className={'video-container'} style={{flex:1, ...props.style as Object}}>
    </div>
}
